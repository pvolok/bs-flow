...
and p_sig s = Types.(function
  ...
  | Sig_type (id, type_decl, _) ->
      Flow_env.add_type s.env (print_id s id) type_decl
  | Sig_typext _ -> ()
  | Sig_module (id, mod_decl, _) ->
      begin match mod_decl.md_type with
      | Mty_signature sigs ->
          Flow_env.enter_module s.env (Ident.name id);
          p_sigs s sigs;
          Flow_env.leave_module s.env
      | _ -> ()
      end
  | Sig_modtype _ -> ()
  | Sig_class _ -> ()
...
let print_signature (env: Flow_env.t) sigs =
  let s = {
    env;
    used_types = [];
    type_ids = SMap.empty;
